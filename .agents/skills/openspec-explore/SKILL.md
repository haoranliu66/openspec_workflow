...
```bash
node bin/openspec.js list --json
```
...
1. **Resolve and read existing artifacts for context**
   - Run `node bin/openspec.js status --change "<name>" --json`.
...
- **Don't manually scaffold changes** - Never create a new change directory under `openspec/changes/` by hand. Always use `node bin/openspec.js new change "<name>"` (with `--store <id>` when applicable) so required metadata such as `.openspec.yaml` is created before writing artifacts.
