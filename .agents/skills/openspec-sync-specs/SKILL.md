...
1. **Select the change**
   ...
   - If ambiguous, run `node bin/openspec.js list --json` to get available changes and ask the user to select one
...
4. **For each delta spec, apply changes to main specs**
   ...
   - If archive invoked this workflow inline and supplied a valid snapshot from
     `node bin/openspec.js instructions specs --change "<name>" --json`, reuse it and do not
   ...
   c. **Apply changes intelligently**:
      ...
      **`## Purpose` in the delta:**
      - The main spec already has one and it is authoritative - leave it alone
        (this is what `node bin/openspec.js archive` does; it warns and moves on)

   d. **Create new main spec** if capability doesn't exist yet:
      - Create `<planningHome.root>/openspec/specs/<capability-path>/spec.md`
      - Add Purpose section: copy the delta's `## Purpose` body verbatim when it has one
        (this is what `node bin/openspec.js archive` does); only write a brief TBD placeholder when it does not
      - Add Requirements section with the ADDED requirements
      - Follow the **Main Spec Format Reference** below

5. **Validate updated main specs**

   Run `node bin/openspec.js validate --specs` with the same selected-root flags used earlier.
...
- A MODIFIED block carries the whole requirement - body plus every scenario that survives the change. `node bin/openspec.js validate` and `node bin/openspec.js archive` both reject one that drops a scenario the main spec still has.
