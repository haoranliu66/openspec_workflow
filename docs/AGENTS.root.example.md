...
对代码、产品行为或系统行为的变更，先阅读并遵守 `docs/AI_WORKFLOW_AGENTS.md`，并以 `docs/FULLSTACK_WORKFLOW.md` 的“统一生命周期”为唯一完整流程。本文件只合并项目级 AI 执行约束，不复制或改写 `/opsx:explore`、路径选择、规划、授权、实施、审核和 formal close 的完整规则。
