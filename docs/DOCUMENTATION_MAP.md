...
| `openspec/change-history.json` | 生成历史 | v2 仅保存 archived 演变摘要；不保存 active change、artifact 路径、验证或证据。不得手工编辑。 |
| `openspec/specs/**` | 当前规格 | 当前行为的最终事实来源；只通过新的 delta change 演进。 |
| `CHANGELOG.md` | 历史摘要 | 按版本说明公开契约演变；历史条目保持当时事实，不作为当前命令说明。 |

## 本地开发记录与发布边界

本工作流源仓库会在开发时产生以下详细记录，但最新公开 checkout 不跟踪其内容：
...
正式关闭先让 history v2 捕获归档摘要，再由根目录跟踪规则保持公开快照精简。这个变更只清理最新 checkout，不重写旧 Git 提交；旧过程文件仍可能从历史提交恢复。
...
5. formal close 后确认 `SPEC.md` 与 history v2 已重建，且公开导航不依赖详细 archive 路径。
